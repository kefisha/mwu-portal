...
let pendingStudents = []; 
let assessments = []; 
let adminNotifications = []; // Tracks all activities for Admin review
...
let sectionSchedules = {
    "1st Year - Section A": [
        ...
        { course: "Engl 101: Communicative English", teacher: "Dr. Sara", time: "Wed 03:00 - 05:00 AM", room: "Lab-1" }
    ],
    "1st Year - Section B": [
        { course: "Comp 101: Intro to Computer Science", teacher: "Abebech Bekele", time: "Mon 08:00 - 10:00 AM", room: "Lab-3" },
        { course: "Stat 101: Introduction to Statistics", teacher: "Dr. Teshale Kebede", time: "Thu 03:00 - 05:00 AM", room: "Hall-01" }
    ]
};
...
app.post('/api/student-self-register', registerUpload, (req, res) => {
    ...
    pendingStudents.push({
        id: newTempId, student_id: autoID, password: autoPIN,
        name: name.trim(), father_name, mother_name, gender, age, phone,
        department: department.trim(), class_level: assignedSection, bank_slip_val: txn_id, photo: photoPath
    });
...
app.post('/login', (req, res) => {
    ...
    if (role === 'admin' && uKey === ADMIN_USER && password === ADMIN_PASS) {
        ...
        if (teacherAccounts[uKey] && teacherAccounts[uKey].pass === password.trim()) {
...
// ADMIN DASHBOARD (NOTIFICATIONS + EDIT & MANAGE)
app.get('/admin', (req, res) => {
    if (!req.session.isAdminLoggedIn) return res.redirect('/');

    let pendingRows = pendingStudents.map(st => `
        <tr>
            <td>${st.student_id}</td><td><b>${st.password}</b></td><td>${st.name}</td><td>${st.class_level}</td>
            <td><a href="/admin/approve-student/${st.id}" style="background:green; color:white; padding:4px 8px; text-decoration:none; border-radius:4px;">✅ Approve</a></td>
        </tr>
    `).join('') || '<tr><td colspan="5">ምንም አዲስ ጥያቄ የለም</td></tr>';

    let teacherRows = Object.values(teacherAccounts).map(t => `
        <tr><td>${t.id}</td><td><b>${t.pass}</b></td><td>${t.name}</td><td>${t.dept}</td><td>${t.assigned_section}</td>
        <td><a href="/admin/edit-teacher/${t.id}" style="background:#2980b9; color:white; padding:3px 6px; text-decoration:none; border-radius:4px;">✏️ Edit</a></td></tr>
    `).join('');

    let studentRows = Object.values(studentData).map(st => `
    ...
    let notifRows = adminNotifications.map(n => `
    ...
    <style>body { font-family: sans-serif; background: #eef2f5; padding: 15px; font-size: 13px; } .card { background: white; padding: 15px; border-radius: 10px; margin-bottom: 20px; } table { width: 100%; border-collapse: collapse; margin-top:5px; } th, td { border: 1px solid #ddd; padding: 6px; text-align: center; } th { background: #34495e; color: white; }</style>
    </head>
    <body>
        ...
        <div class="card">
            <h3>📥 አዲስ የተመשገቡ ተማሪዎች (Pending Registration Approval)</h3>
            <table><thead><tr><th>ID</th><th>Password</th><th>Name</th><th>Class</th><th>Action</th></tr></thead><tbody>${pendingRows}</tbody></table>
        </div>
        <div class="card">
            <h3>👨‍🏫 የመምህራን ዝርዝር (Manage & Edit Teachers)</h3>
            <table><thead><tr><th>ID</th><th>Pass</th><th>Name</th><th>Dept</th><th>Section</th><th>Action</th></tr></thead><tbody>${teacherRows}</tbody></table>
        </div>
        <div class="card">
            <h3>🎓 የጸደቁ ተማሪዎች ዝርዝር (Manage & Edit Students)</h3>
            <table><thead><tr><th>ID</th><th>Pass</th><th>Name</th><th>Class</th><th>Action</th></tr></thead><tbody>${studentRows}</tbody></table>
        </div>
...
app.get('/admin/approve-student/:id', (req, res) => {
    ...
    if (index !== -1) {
        ...
        studentData[st.student_id] = { ...st, status: "Approved", admin_message: "🎉 ምዝገባዎ ጸድቋል!" };
...
app.get('/admin/edit-teacher/:id', (req, res) => {
    if (!req.session.isAdminLoggedIn) return res.redirect('/');
    let t = teacherAccounts[req.params.id];
    ...
    <div style="font-family:sans-serif; padding:30px; max-width:400px; margin:auto;">
        ...
        <form action="/admin/update-teacher/${t.id}" method="POST">
...
app.post('/admin/update-teacher/:id', (req, res) => {
    if (!req.session.isAdminLoggedIn) return res.redirect('/');
    let t = teacherAccounts[req.params.id];
    if (t) {
        t.name = req.body.name;
        t.pass = req.body.pass;
        t.assigned_section = req.body.assigned_section;
    }
    res.redirect('/admin');
});
...
app.get('/admin/edit-student/:id', (req, res) => {
    if (!req.session.isAdminLoggedIn) return res.redirect('/');
    let st = studentData[req.params.id];
...
app.post('/admin/update-student/:id', (req, res) => {
    ...
    if (st) {
        ...
        st.password = req.body.password;
        st.class_level = req.body.class_level;
        studentAccounts[st.student_id] = req.body.password;
    }
    res.redirect('/admin');
});

// TEACHER DASHBOARD (CREATING ASSESSMENT NOTIFIES ADMIN)
app.get('/teacher-dashboard', (req, res) => {
    if (!req.session.teacherId) return res.redirect('/');
    let teacher = teacherAccounts[req.session.teacherId];
    let assignedSec = teacher.assigned_section;
    let secAssessments = assessments.filter(a => a.section === assignedSec);

    let assessmentRows = secAssessments.map(a => `
        <div style="background:#f9f9f9; padding:8px; margin-bottom:8px; border-left:3px solid green;">
            <b>${a.title}</b> - ${a.deadline}
        </div>
    ...
    <style>body { font-family: sans-serif; background: #eef2f5; padding: 20px; } .card { background: white; padding: 20px; border-radius: 10px; max-width: 600px; margin: auto; }</style>
    </head>
    <body>
        <div class="card">
            <h2>👨‍🏫 መምህር ${teacher.name} ዳሽቦርድ</h2>
            <p>ሴክሽን: <b>${assignedSec}</b></p>
            <hr>
            ...
            <form action="/teacher/create-assessment" method="POST">
                ...
                <button type="submit" style="background:#27ae60; color:white; border:none; padding:10px; width:100%; border-radius:6px; font-weight:bold;">📤 ለተማሪዎች ላክ (Notification to Admin)</button>
            </form>
            <hr>
            <h3>📋 የለቀቋቸው Assessments</h3>
            ${assessmentRows}
        </div>
...
app.post('/teacher/create-assessment', (req, res) => {
    if (!req.session.teacherId) return res.redirect('/');
    const { section, title, description, deadline } = req.body;
    let teacher = teacherAccounts[req.session.teacherId];

    assessments.push({ id: Date.now(), section, teacherName: teacher.name, title, description, deadline });
...
app.get('/student-dashboard', (req, res) => {
    ...
    let secAssessments = assessments.filter(a => a.section === student.class_level);
    ...
    <style>body { font-family: sans-serif; background: #f4f7f6; padding: 20px; } .container { max-width: 600px; margin: auto; background: white; padding: 20px; border-radius: 10px; }</style>
    </head>
    <body>
        <div class="container">
            <h2>🎓 የተማሪ ዳሽቦርድ</h2>
